...
public class ComplianceRecordController {
    ...
    public ResponseEntity<ComplianceResponseDTO> createCompliance(@Valid @RequestBody ComplianceResponseDTO requestDto) {
        ...
        ComplianceResponseDTO response = service.createCompliance(requestDto);
        return new ResponseEntity<>(response, HttpStatus.CREATED);
    }
    
    @PostMapping("/addNewProgram")
    public ResponseEntity<CulturalProgramResponseDto> addNewProgram(@Valid @RequestBody CulturalProgramResponseDto requestDto) {
        logger.info("REST Request: Create new Cultural Program with title: {}", requestDto.getTitle());
        
        CulturalProgramResponseDto createdProgram = service.addNewProgram(requestDto);
